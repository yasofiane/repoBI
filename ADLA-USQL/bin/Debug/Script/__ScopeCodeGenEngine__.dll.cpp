...
// Explicit instantiations for templated functions that are defined in managed code
    ...
    public:
        ...
        static const std::string GetSchema()
        {
            return std::string{ u8"customer:string,amount:double" };
        }
...
// project, and then turn off /doc.
    ...
    {
    public:
        static const unsigned __int64 m_cRows = 2;
...
// project, and then turn off /doc.
    ...
    {
        "new System.String[]{\"Contoso\",\"Woodgrove\"}[(int)(RowNumber0__ - 1L)]",
        "new System.Double[]{1500.0,2700.0}[(int)(RowNumber0__ - 1L)]",
    ...
    public:
        ...
        static bool FilterTransformRow(Process_1_Data0 & input, Process_2_Data0 & output, IncrementalAllocator * alloc)
        {
            ...
            try
            {
                    output.m_customer = staticarray_0[scope_cast<int>((scope_cast<__int64>(input.m_RowNumber0__) - 1LL))];
                    exceptionIndex++;
                    output.m_amount = staticarray_1[scope_cast<int>((scope_cast<__int64>(input.m_RowNumber0__) - 1LL))];
                    exceptionIndex++;
                    return true;
                    }
                    catch(...)
        ...
        static const char * const staticarray_0_str[2];
        static const std::uint32_t staticarray_0_len[2];
        static const StaticArrayHelper<double> staticarray_1;
        static const double staticarray_1_val[2];
            #pragma warning(push)
            #pragma warning(disable: 4100) // 'alloc': unreferenced formal parameter
            static void InitializeStatics(IncrementalAllocator * alloc)
            {
        ...
        const char * const RowTransformPolicy<Process_1_Data0, Process_2_Data0, UID_Process_2>::staticarray_0_str[2] =
        {
            "Contoso", "Woodgrove"
        };
        const std::uint32_t RowTransformPolicy<Process_1_Data0, Process_2_Data0, UID_Process_2>::staticarray_0_len[2] =
        {
            7U, 9U
        };
        const StaticArrayHelper<double> RowTransformPolicy<Process_1_Data0, Process_2_Data0, UID_Process_2>::staticarray_1
        {
            RowTransformPolicy<Process_1_Data0, Process_2_Data0, UID_Process_2>::staticarray_1_val
        };
        const double RowTransformPolicy<Process_1_Data0, Process_2_Data0, UID_Process_2>::staticarray_1_val[2] =
        {
            1500.0, 2700.0
        };
...
// project, and then turn off /doc.
    ...
    public:
        ...
        {
            if (!row.m_customer.IsNull())
            {
                output->Write<FString,true>(row.m_customer);
            }
            output->WriteDelimiter();
            output->Write<double,false>(row.m_amount);
            output->WriteNewLine();
        }
